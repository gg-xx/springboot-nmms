    ...
    <script>
        $(function () {
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        if (result.responseCode == "2001") {
                            location.href = "${pageContext.request.contextPath}/type/findAll?pageNo=${pageTypes.pageNum}";
                        } else {
                            $("#errorMsg").tooltip({
                                title: "error",
                                placement: "center",
            ...
            $("input[name='modifyStatus']").click(function(){
                $.ajax({
                    type:"post",
                    url:"${pageContext.request.contextPath}/type/modifyStatus",
                    data:{"id":$(this).attr("data-id")},
                    dataType:"json",
                    success:function(result){
                        if (result.responseCode == "2001") {
                ...
                <c:forEach items="${pageTypes.list}" var="type">
                    <tr>
                        ...
                        <td class="text-center">
                            <input type="button" class="btn btn-warning btn-sm doProTypeModify" name="showModify" data-id="${type.id}" value="修改">
                            <c:if test="${type.status == 1}">
                                <input type="button" class="btn btn-danger btn-sm doProTypeDisable" name="modifyStatus" data-id="${type.id}" value="禁用">
                            </c:if>
                            <c:if test="${type.status == 0}">
                                <input type="button" class="btn btn-success btn-sm doProTypeDisable" name="modifyStatus" data-id="${type.id}" value="启用">
                            </c:if>
